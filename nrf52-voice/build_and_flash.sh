#!/bin/bash
# Build nrf52-voice with MCUboot OTA support and flash over USB (UF2).
#
...
OTA_BIN="$BUILD_DIR/nrf52-voice/zephyr/zephyr.signed.bin"
if [ -f "$OTA_BIN" ]; then
...
if [ -f "$MERGED_UF2" ]; then
    ...
    for _i in $(seq 1 60); do
        ...
        if [ -d /Volumes/XIAO-SENSE ]; then
            ...
            echo "OTA update: python3 mac_client/ota_updater.py nrf52-voice/ota_update.bin"
            exit 0
        fi
