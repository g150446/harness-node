...
| 録音トリガー | BLE クライアントからの write のみ（モーション連動は無効） |
...
録音の開始・停止は **BLE クライアントからの Audio RX write のみ** で行う。モーション検出は BLE 通知を送るが、録音には影響しない。
...
モーション連動録音を再有効化する場合は `src/main.c` の `process_motion_sample()` 末尾に `on_motion_started()` / `on_motion_settled()` の呼び出しを戻す。
...
LSM6DS3TR-C のハードウェア wakeup 検出を I2C 直接アクセスで設定。26Hz ポーリングループ内で `WAKE_UP_SRC` レジスタを確認し、閾値超過時に Wakeup TX 通知を送信する。
...
| `MD1_CFG` | 0x5E | `0x20` | INT1_WU のみルーティング |
...
ジェスチャー判別は **Mac クライアント（Python）側** で完結する。ファームウェアは Motion TX と Wakeup TX の生データを送るのみ。

### 判別アルゴリズム

MOTION SETTLED 受信時に評価する：

```
MOTION ACTIVE 受信後に Wakeup TX が 1 回以上届いた
  → DOUBLE_CLENCH（ダブルクレンチ）

MOTION ACTIVE 受信後に Wakeup TX がない
  → ARM_LIFT（腕を持ち上げる）
```

**物理的根拠:**
- **ARM_LIFT**: 腕を持ち上げる動作で wakeup が複数発生することがあるが、それらはすべて ACTIVE 検出**前**（腕を動かし始めた瞬間）に発生する
- **DOUBLE_CLENCH**: 2 回目のクレンチが ACTIVE 検出**後**に発生するため、ACTIVE の後に必ず wakeup が来る

**補助特徴量（z_excursion_peak）:**
- `z_excursion_peak` = モーション開始から SETTLED までの `|z − baseline_z|` 最大値（m/s²）
- 腕を持ち上げると重力ベクトルの Z 成分が変化するため、arm_lift で大きな値が出ることがある
- 現在のロジックでは主判定（wakeup_after_active）の補助として参照可能
...
### 主要定数（nrf52_voice_client.py）

```python
WAKEUP_COUNT_DOUBLE_CLENCH = 2    # ACTIVE後にwakeupが1回以上 → DOUBLE_CLENCH
Z_EXCURSION_LIFT_THRESH    = 5.0  # m/s² (補助: 大きなZ偏差はarm_liftと判定)
```
