...
| Power State TX | `...0015...` | Notify | 低電力モード移行／復帰通知 |
...
**Power State TX パケット形式（1 バイト）:**

```
Byte 0: state  0x01 = 低電力モードへ移行 / 0x00 = 低電力モードから復帰
```
...
| `EVENT_POLL_INTERVAL_MS` | 25 | Wakeup / Tilt レジスタ確認周期（ms）（通常動作時） |
| `IDLE_SLEEP_TIMEOUT_MS` | 30000 | この時間（ms）イベントがなければ低電力モードへ移行 |
| `SLEEP_POLL_INTERVAL_MS` | 1000 | 低電力モード中のポーリング周期（ms） |
...
30 秒間イベント（wakeup・tilt・BLE write・接続）が発生しない場合、メインループのポーリング間隔を 25ms → 1000ms に延長する。Zephyr は `k_sleep()` 中にアイドルスレッドが自動的に WFI を実行するため、CPU の起床頻度が下がり消費電力が削減される。BLE スタックはハイプライオリティ割り込みで動作するため、低電力モード中も接続・アドバタイジングは維持される。

### 動作フロー

```
[通常動作]
  最後のイベントから 30 秒経過 & 非録音
    ↓
  low_power_mode = true
  printk(">>> Entering low-power mode (idle 30s)")
  Power State TX に 0x01 を通知
  [SLEEP] Device entering low-power mode
    ↓
  メインループ: k_sleep(1000ms)  — CPU の WFI 滞在時間が増加
  音声スレッド: k_msleep(1000ms)
  BLE 接続・アドバタイジングは BLE スタックが独立維持

[イベント発生時（wakeup / tilt / BLE write / 接続）]
  reset_idle_timer():
    low_power_mode = false
    printk(">>> Waking from low-power mode")
    Power State TX に 0x00 を通知
    [SLEEP] Device waking up from low-power mode
    ↓
  メインループ: k_sleep(25ms) に戻る
  last_event_time_ms リセット → 30 秒タイマー再スタート
```

### イベント計測対象

`reset_idle_timer()` を呼ぶトリガー（どれか一つで低電力モード解除 & タイマーリセット）:

| トリガー | 呼び出し箇所 |
|----------|------------|
| IMU wakeup 検出 | `check_wakeup_src()` — `WU_IA` 検出直後 |
| IMU tilt 検出 | `check_tilt_src()` — `TILT_IA` 検出直後 |
| モーション開始 | `on_motion_started()` 先頭 |
| BLE write 受信 | `audio_rx_write()` — START/STOP 両方 |
| BLE 接続確立 | `ble_connected()` — 接続直後 |

### WDT との整合性

`SLEEP_POLL_INTERVAL_MS = 1000ms` < `WDT_TIMEOUT_MS = 5000ms` のため、低電力モード中もウォッチドッグのキックは問題なく行われる。
...
> 低電力モード（30秒アイドル後）ではポーリング間隔が 25ms → 1000ms に延長され、CPU の WFI 滞在時間が増加する。BLE 接続維持に必要な最小動作は BLE スタックが担うため、アプリ側 CPU はほぼスリープ状態になる。
