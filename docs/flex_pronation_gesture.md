...
### 実機1試行での確認（2026-09-01 07:58、session=143 `入力テスト`）
...
**手首が静止している 500 ms の hold 中に、現行推定器は挙上インパルスを 1.46 積む。**
最終ゲート値 2.562 の過半が、動作終了後の重力推定の追従遅れで生じている。
τ=0.30 s は flip 後 500 ms でもまだ緩和しきっていない。
gyro 伝播版は同区間で +0.024 しか増えず、逆に本来の挙上区間では大きい値
（1.602 vs 1.098）を返す — ハイパスが削っていた成分を回収している。
（実機は n=1。傾向の裏付けであって統計ではない。）
