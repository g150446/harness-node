...
対象ファームウェア: HarnessNode `0.0.32` 以降
...
- 回内成立後 1500 ms 以内に、逆符号の `gyro_y`（|gyro_y| ≥ 12°/s）で回外を開始する
...
| 回内積分角 | 30° 以上 |
| 回内ピーク | 40°/s 以上 |
| 回外開始 \|gyro_y\| | 12°/s 以上・回内と逆符号 |
| 回外積分角 | **不要** |
| 回外開始期限（回内後） | 1500 ms |
| 回内／回外最長 | 各 2000 ms |
| 最終 Y 比 | 0.94 以上（垂直 ±約 20°） |
| 最終静止 | 角速度 50°/s 以下、線形加速度 2.2 m/s² 以下を 500 ms |
| 全シーケンス期限 | 5000 ms |
...
本番ビルドでは `GESTURE_DEBUG_GYRO_Y` を `0` に戻して再ビルド・OTA する。
...
USB 書き込みは行わない。更新後、`0.0.32` が slot 0 で `active` かつ `confirmed`
であることを確認する。
