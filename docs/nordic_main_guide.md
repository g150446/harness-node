...
| LED 方針 | 未接続アドバタイズ中は青の遅い点滅。接続待機は消灯。録音中のみ赤 |
| 署名バージョン（目安） | `0.0.99+`（primary idle 30–50 ms。未接続 3 分で SYSTEMOFF） |
...
接続中の消費は **BLE 間隔** が支配的。未接続の長時間放置は **SYSTEMOFF**（`0.0.98+`）。IMU ポールを 25→50 ms にするライトスリープは効果が小さい。

#### ライトスリープ

10 秒間 `motion_active` でなく録音中でもない場合、IMU ポーリング間隔だけ落とす。BLE 接続は維持。
...
録音停止後もタイマーはリセットされ、即座には入らない。

#### 接続中の BLE 間隔

録音していない primary は、タップも 10 秒なければ 7.5–15 ms から **30–50 ms** へ落とす。secondary は常に 200–500 ms。接続直後も fast は要求しない。

`0.0.98` は idle を 200–500 ms にした。接続パラメータ更新の instant は「今の間隔 × 約 6 イベント」なので、タップから 7.5 ms に戻るまで 1–3 秒かかり、録音開始が約 2 秒遅れた。30–50 ms は 7.5 ms 比でラジオ回数を 4–7 倍減らしつつ、その遅延を避ける。

| 条件 | primary | secondary |
|------|---------|-----------|
| 接続直後 / 10 秒無タップ・非録音 | 30–50 ms | 200–500 ms |
| タップまたは録音開始から 10 秒以内 | 7.5–15 ms | 200–500 ms |
| 録音中 | 7.5–15 ms | 200–500 ms |

タップまたは録音開始で fast に戻す。アイドル中のタップ通知は最大約 50 ms 遅れる。

#### 未接続 SYSTEMOFF

接続 0 のまま 3 分続き、かつ USB（VBUS）が無いとき、アドバタイズを止めて nRF SYSTEMOFF に入る。

- IMU はダブルタップだけを INT1 に出す。単タップでは起きない
- 起床はリセット相当。通常ブートのあとアドバタイズと青点滅を再開する
- USB 給電中は入らない（デバッグ用）
- SYSTEMOFF 中の床は IMU accel（タップ用 416 Hz）が支配的。nRF 本体は µA
...
| `SLEEP_IDLE_TIMEOUT_MS` | 10000 ms | ライトスリープ移行、および接続中 BLE 間隔を落とすまでの無タップ時間 |
| `SLEEP_POLL_INTERVAL_MS` | 50 ms | ライトスリープ中の IMU ポーリング間隔 |
| `ADV_NO_CONN_SLEEP_MS` | 180000 ms | 未接続アドバタイズ後に SYSTEMOFF するまでの時間 |
| `ADV_LED_ON_MS` / `ADV_LED_OFF_MS` | 200 / 1800 ms | 未接続アドバタイズ中の青点滅 |
...
|------|----------------|
| 起動直後 | 白（1 秒点灯） |
| BLE アドバタイジング中（接続 0） | 青 200 ms ON / 1800 ms OFF |
| BLE 接続済み（待機中） | 消灯 |
| 単純なモーション検出中 | 消灯 |
| 録音中 | 赤（常時点灯） |
| 未接続 3 分後 SYSTEMOFF | 消灯 |

接続 0 のアドバタイズ中だけ青を遅い点滅にします。接続が 1 本でもある間（2 本目待ちのアドバタイズ含む）は消灯です。録音中は赤。赤消灯は `is_recording == false` と一致し、意図的に録音継続のまま LED だけ消す経路はない。
