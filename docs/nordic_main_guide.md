...
| 署名バージョン（目安） | `0.0.98+`（接続アイドルで BLE 間隔を落とす。未接続 3 分で SYSTEMOFF） |
...
接続中の消費は **BLE 間隔** が支配的。未接続の長時間放置は **SYSTEMOFF**。IMU ポールを 25→50 ms にするライトスリープは効果が小さい。
...
録音していない primary は、タップも 10 秒なければ 7.5–15 ms から 200–500 ms へ落とす。secondary は常に遅い間隔。接続直後も fast は要求しない。

| 条件 | primary | secondary |
|------|---------|-----------|
| 接続直後 / 10 秒無タップ・非録音 | 200–500 ms | 200–500 ms |
| タップまたは録音開始から 10 秒以内 | 7.5–15 ms | 200–500 ms |
| 録音中 | 7.5–15 ms | 200–500 ms |

タップまたは録音開始で fast に戻す。アイドル中のタップ通知は最大約 0.5 s 遅れることがある。
