...
XIAO nRF52840 Sense の充電IC **BQ25101** は以下の設定で動作する：

| 項目 | 値 |
|------|-----|
| 充電終止電圧（VBATREG） | 4.2V |
| デフォルト充電電流（HICHG=HIGH） | **50mA** |
| 終止電流（ITERM） | 充電電流の10% = **5mA** |

充電器は CV 相（4.2V 定電圧）でバッテリー端子を4.2Vに保持するが、これはあくまで**充電器が印加している電圧**である。ITERM（5mA）で充電終了後、USB を外すとバッテリーの**開路電圧（OCV）は 4.08〜4.15V 付近に落ち着く**。

旧LUTは充電器印加電圧（4.2V）を100%基準にしていたため、満充電後のUSB抜きで即座に88%前後を示していた。新LUTはOCVを基準とし、満充電後の実測値（≈4080mV）が適切に95〜100%の範囲に収まるよう調整している。
