...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの手の甲側に配置し、短く1回振り、手のひらを上へ向け、手を上げてから掌を下にして0.5秒静止すると録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
録音開始は、以下が順番に成立したとき（加速度のみ。ジャイロは使わない）:

1. **掌下シェイク**（甲側装着）: 基板水平（Z 絶対比 ≥ 0.80）で、重力直交の線形加速度の峰間 ≥ 5.0 m/s² かつ `|平均| < 0.4 × 峰間`
2. **掌上**: シェイク窓開始時の重力 LP 基準から、3D 傾き ≥ 15°、phi ≥ 8°、Δz 比 ≥ 0.25、または Z 符号反転
3. **挙上**: 上向き加速度パルス。掌向きは問わない
4. **掌下で静止**: 掌上基準から手首を回し（phi ≥ 20° / Δz 比 0.50 / 符号反転）、線形加速度 RMS が低く姿勢差 ≤ 10° を 500 ms

回外・最終仰角帯・甲上条件は要求しません。しきい値と状態遷移の正本は
`docs/flex_pronation_gesture.md`、保守上の注意は
`docs/lift_detection_maintenance.md`。

録音停止は、録音開始時の掌下姿勢からの緩い掌上反転（3D 15° / phi 8° / Δz 0.25 / Z 符号）。手を下ろすだけでは止まらない。
