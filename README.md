...
│   ├── gesture_validator.py   # BLE接続で回内→直後回外→垂直静止を検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの掌側に配置し、手のひら上向き水平から回内し、直後に回外して手・前腕を垂直（±20°）で0.5秒静止すると録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
1. **掌上水平開始**: 補正Z投影比 ≥ 0.90、Y投影比の絶対値 ≤ 0.30、角速度 ≤ 70°/s で120 ms静止
2. **回内**: `gyro_y` 積分角 ≥ 30°、ピーク ≥ 40°/s
3. **直後の回外**: 回内後1500 ms以内に逆符号の `gyro_y`（|gyro_y| ≥ 12°/s）で開始（積分角は不要）
4. **垂直静止**: Y投影比の絶対値 ≥ 0.94（垂直から約20°以内）で500 ms安定

回内／回外の相対符号で右手・左手とUSB向きの差に対応します。肘屈曲角・5 cm距離・Y重力維持は要求しません。
...
回内→直後回外→垂直静止ジェスチャーを試行ごとに検証する場合:
...
開始姿勢、静止、回内、直後の回外、垂直姿勢、垂直保持、途中リセット、最終発動を
