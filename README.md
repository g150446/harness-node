...
│   ├── gesture_validator.py   # BLE接続で水平回内→屈曲・回外→垂直静止を検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの掌側に配置し、手のひら上向き水平から前腕を水平に保ったまま回内し、続けて肘屈曲＋回外を行い、手・前腕を垂直位置で0.5秒静止すると録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
録音開始は、以下の **5段階**が順番に成立したとき:

1. **掌上水平開始**: 補正Z投影比 ≥ 0.90、Y投影比の絶対値 ≤ 0.17（±10°）で200 ms静止
2. **水平回内**: 前腕水平を維持したままY軸回転角 ≥ 30°、往路中の最大|Y比| ≤ 0.17
3. **切り返し**: 750 ms以内に回転停止またはY軸回転方向の反転を検出
4. **肘屈曲＋回外＋距離**: X-Z合成回転角 ≥ 60°、Y軸回転は往路と逆符号、融合推定距離 ≥ 5 cm
5. **垂直静止**: Y投影比の絶対値 ≥ 0.94（垂直から20°以内）で500 ms安定

往路と復路の回内／回外符号を相対比較するため、右手・左手とUSB端子の向きが逆の装着に対応します。手のひら下向き開始、単純な屈曲、回内または回外だけでは発動しません。
...
水平回内→屈曲・回外→垂直静止ジェスチャーを試行ごとに検証する場合:
...
開始姿勢、静止、水平回内、切り返し、屈曲、回外、5 cm移動、垂直姿勢、垂直保持、途中リセット、最終発動を
試行ごとに `[OK]` / `[NG]` / `[--]` で表示します。BLE診断の生ログは画面へ出さず、
指定時だけJSONレポートへ保存します。詳細とCodexからの起動方法は
