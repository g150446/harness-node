...
1. **掌上開始**（甲側装着）: 補正Z投影比 ≥ 0.90、角速度 ≤ 70°/s で120 ms静止（開始時の|Y|上限なし）
2. **回内**: `gyro_y` 積分角 ≥ 15°、ピーク ≥ 30°/s
3. **重力に逆らう上昇**: IMU積分ピーク ≥ 1.5 cm（物理約5 cm前後の上昇に相当）
4. **静止保持**: 線形加速度 ≤ 3.0 m/s² を500 ms（最終静止に角速度は使わない）

`GESTURE_PALM_UP_Z_SIGN = -1`（甲側・部品面皮膚の実測）。回外・最終仰角帯・甲上条件は要求しません。
