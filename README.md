...
│   ├── gesture_validator.py   # BLE接続で屈曲→回内を試行単位にPASS/FAIL検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。肘を伸ばした姿勢から屈曲し、その後に手関節・前腕を回内するジェスチャーで録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
録音開始は、200 ms 以上の角速度安定後に以下の **2 段階**が順番に成立したとき:

1. **肘屈曲**: 前腕長手軸に直交する Y-Z 平面の回転角 ≥ 35°、ピーク角速度 ≥ 50°/s、加速度エビデンス ≥ 0.5 m/s²（180〜1600 ms）
2. **回内**: 屈曲成立から 1500 ms 以内に、前腕長手 X 軸まわりの回転角の絶対値 ≥ 30°、ピーク角速度 ≥ 55°/s（3 サンプル以上）

X 軸まわりの角度は正負を別々に積算して大きい側を判定するため、小さな揺り戻しで相殺されず、右手・左手のどちらでも発動します。重力の向きや静定時の特定軸を条件にしないため、立位・寝た姿勢・走行中の車内でも同じ判定を使用します。自然な動作で回内成分が先に見える場合に備え、静止成立後1000 ms以内のX軸成分を保持しつつ、最終的にはY-Z屈曲角、ピーク速度、加速度エビデンスをすべて要求します。成立直後の腕戻しは、直前の回内と逆向きのX軸回転として除外します。
...
カウントダウン後にMacのスピーカーからPing音が鳴ります。Ping後に動作すると、BLEの `recording_start` 受信有無、判定遅延、
PASS/FAIL、全試行の集計を表示します。詳細とCodexからの起動方法は
