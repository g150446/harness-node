...
│   ├── gesture_validator.py   # BLE接続で水平→屈曲→垂直を試行単位にPASS/FAIL検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの掌側に配置し、手のひらが水平な状態から5 cm以上移動するよう肘を屈曲して、手・前腕を垂直位置に留めると録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
録音開始は、以下の **3段階**が順番に成立したとき:

1. **水平開始**: 掌面法線Z軸の重力投影比 ≥ 0.80で200 ms静止
2. **肘屈曲と距離**: 掌面内X-Y合成回転角 ≥ 60°、ピーク角速度 ≥ 50°/s、加速度エビデンス ≥ 0.5 m/s²（180〜2000 ms）に加え、加速度とジャイロの融合推定距離 ≥ 5 cm
3. **垂直静止**: X-Y面の重力投影比 ≥ 0.80、Z軸の投影比 ≤ 0.45で、角速度と線形加速度が250 ms安定

姿勢軸は絶対値とX-Y合成値で判定するため、右手・左手、掌の上向き・下向き、リストバンド内の基板面内回転に対応します。回内・回外の角度、方向、速度は発動条件に含みません。
...
水平開始→肘屈曲→垂直静止ジェスチャーを試行ごとに検証する場合:
...
開始姿勢、静止、屈曲、5 cm移動、垂直姿勢、垂直保持、途中リセット、最終発動を
