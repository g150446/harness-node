...
│   ├── gesture_validator.py   # BLE接続でシェイク→掌上→挙上→掌下静止を検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの手の甲側に配置し、短く1回振り、手のひらを上へ向け、手を上げてから掌を下にして約0.4秒静止すると録音を自動開始します（加速度常時 + ジャイロはシェイク成立〜録音終了のみ）。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
録音開始は、以下が順番に成立したとき（加速度常時 + ジャイロはシェイク成立〜録音終了のみ）:

1. **掌下シェイク**（甲側装着）: 基板水平（Z 絶対比 ≥ 0.80）で、重力直交の線形加速度の峰間 ≥ 5.0 m/s² かつ `|平均| < 0.4 × 峰間` → ジャイロ ON
2. **掌上**: 重力ゲート **または** gyro_y（前腕軸）の積分/ピーク
3. **挙上**: 上向き加速度パルス。掌向きは問わない
4. **掌下で静止**: 重力反転 **または** 逆符号 gyro_y のあと、線形加速度 RMS≤3.0 を 400 ms（進入時のみ \|ω_y\|≤90 dps）

回外・最終仰角帯・甲上条件は要求しません。しきい値と状態遷移の正本は
`docs/flex_pronation_gesture.md`、保守上の注意は
`docs/lift_detection_maintenance.md`。

録音停止は、開始姿勢からの掌上（重力 **または** gyro_y）。手を下ろすだけでは止まらない。停止後ジャイロ OFF。
...
シェイク→掌上→挙上→掌下静止ジェスチャーを試行ごとに検証する場合:
...
カウントダウン後にMacのスピーカーからPing音が鳴ります。Ping後に動作すると、
掌下シェイク、掌上、挙上、掌下で静止、途中リセット、最終発動、
掌上での録音終了を試行ごとに `[OK]` / `[NG]` / `[--]` で表示します。
GO後の判定時間は既定15秒（開始後の掌上停止を含む）。BLE診断の生ログは画面へ出さず、
指定時だけJSONレポートへ保存します。詳細は
`docs/flex_pronation_gesture.md` を参照してください。
