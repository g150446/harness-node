...
2. **挙上**: 上向き加速度パルス。掌向きは問わない
3. **掌下で静止**: 掌上基準からの重力＋gyro連動条件を試行中ラッチし、RMS≤3.0で進入。保持中はRMS>3.5が2サンプル連続した場合だけ中断し、400 ms維持

回外・最終仰角帯・甲上条件は要求しません。しきい値と状態遷移の正本は
`docs/flex_pronation_gesture.md`、保守上の注意は
`docs/lift_detection_maintenance.md`。

録音停止は、開始後3秒の抑制のあと、掌下基準からの掌上（重力 LP の phi + 強い Δz/符号、
または gyro_y の ∫+peak）を **500 ms 連続** で検出したとき。手を下ろすだけでは止まらない。
停止後ジャイロ OFF。最終発火には挙上正インパルス0.65 m/s以上かつ掌上基準から140°以上の反転を要求します。詳細は `docs/flex_pronation_gesture.md`（現行 `0.0.72`）。
...
既定1.3秒後（`--stop-cue-delay`）のGlass音と`STOP GO`のあとにだけ掌上へ戻します。
STOP GOより前に掌上へ戻す動作は不成立として扱います。`--start-only` では STOP GO を
出さず、ホスト停止で開始のみを判定します。

掌上候補、0.5秒静止、挙上、掌下で静止、途中リセット、最終発動、
掌上での録音終了を試行ごとに `[OK]` / `[NG]` / `[--]` で表示します。
GO後の判定時間は既定15秒。BLE診断の生ログは画面へ出さず、判定履歴をJSON、
6軸履歴をCSV・PNGへ保存し、試行後に時系列グラフを表示します（`--no-plot` で
ウィンドウのみ抑制）。詳細は `docs/flex_pronation_gesture.md`（ファーム `0.0.72`）を
参照してください。
