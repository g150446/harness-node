...
│   ├── gesture_validator.py   # BLE接続で回内→上昇→0.5秒静止を検証
...
PDM マイク音声 + LSM6DS3TR-C ジェスチャー検出 + BLE OTA + **バッテリー残量通知** を統合したファームウェアです。XIAOをリストバンドの手の甲側に配置し、手のひらを上向きに静止してから回内し、重力に逆らって手を上げ（物理目安約5 cm）、その位置で0.5秒静止すると録音を自動開始します。ジェスチャー判別は **ファームウェア側で完結** しており、Mac クライアントは BLE イベント（`0x01` / `0x02`）を受け取るだけです。
...
1. **掌上静止**（甲側装着）: Z 絶対比 ≥ 0.80、線形加速度 ≤ 3.0 m/s² を 50 ms。この時点で基準 phi / az を固定する
2. **回内**: 固定基準からの重力 phi ≥ 8°（完了 20°）、または Z 方向の明確な変化（Δz 比、符号反転、ピーク間）
3. **重力に逆らう上昇**: 上向き加速度パルス（物理目安約 5 cm。変位 cm は積分しない）
4. **静止保持**: 上昇位置で線形加速度 RMS が低く、姿勢差 ≤ 10° を 500 ms
...
回内→上昇→0.5秒静止ジェスチャーを試行ごとに検証する場合:
...
開始姿勢、静止、回内、上昇、静止保持、途中リセット、最終発動を
試行ごとに `[OK]` / `[NG]` / `[--]` で表示します。BLE診断の生ログは画面へ出さず、
