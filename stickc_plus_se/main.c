...
static const char *TAG = "hn_plusse";

/* M5StickC Plus SE pins (docs.m5stack.com/en/core/StickC-Plus_SE). */
...
#define BATTERY_POLL_INTERVAL_US (30ULL * 1000000ULL)
#define GHOST_KICK_US            (15ULL * 1000000ULL)
...
static uint16_t bas_chr_val_handle;
static uint8_t battery_level_pct;
static esp_timer_handle_t battery_timer;
static esp_timer_handle_t ghost_kick_timer;
...
static void ghost_kick_cb(void *arg);
static void store_peer_mac(void);
static void start_ghost_kick_timer(void);
static void stop_ghost_kick_timer(void);
 ...
 * (see docs/stickc_plus_se_guide.md). Idle params (50–100 ms): used whenever
...
static void deferred_idle_conn_params(void *arg)
{
    ...
    if (!audio_subscribed) {
        ESP_LOGI(TAG, "skip deferred idle conn params (no app subscribe)");
        return;
    }
    request_conn_params(false);
}
...
static void store_peer_mac(void)
{
    ...
    if (audio_conn_handle == BLE_HS_CONN_HANDLE_NONE ||
        ...
        display_set_peer(NULL, false, false);
        return;
    }
    memcpy(peer_mac, desc.peer_id_addr.val, 6);
    peer_present = true;
    display_set_peer(peer_mac, true, smp_subscribed && !audio_subscribed);
}

static void stop_ghost_kick_timer(void)
{
    if (ghost_kick_timer != NULL) {
        (void)esp_timer_stop(ghost_kick_timer);
    }
}
...
static void start_ghost_kick_timer(void)
{
    ...
    (void)esp_timer_stop(ghost_kick_timer);
    (void)esp_timer_start_once(ghost_kick_timer, GHOST_KICK_US);
}
...
static const ble_uuid16_t uuid_bas_svc = BLE_UUID16_INIT(0x180F);
static const ble_uuid16_t uuid_bas_chr = BLE_UUID16_INIT(0x2A19);

static int
bas_gatt_access(uint16_t conn_handle, uint16_t attr_handle,
                struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    (void)conn_handle;
    (void)attr_handle;
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
        uint8_t v = battery_level_pct;
        if (os_mbuf_append(ctxt->om, &v, 1) != 0) {
...
static const struct ble_gatt_svc_def bas_svr_svcs[] = {
    {
        ...
        .characteristics = (struct ble_gatt_chr_def[]) {
            {
                .uuid = &uuid_bas_chr.u,
                .access_cb = bas_gatt_access,
                .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_NOTIFY,
                .val_handle = &bas_chr_val_handle,
            },
            {
                0,
            },
...
static void battery_update(void)
{
    ...
    display_set_battery(pct, chg);
    ESP_LOGI(TAG, "Battery: %d mV (%u%%)%s", mv, (unsigned)pct, chg ? " charging" : "");
    if (bas_chr_val_handle != 0) {
        ble_gatts_chr_updated(bas_chr_val_handle);
    }
}
...
ble_app_advertise(void)
{
    ...
    fields.name = (uint8_t *)BLE_ADV_NAME;
    fields.name_len = strlen(BLE_ADV_NAME);
    ...
    rsp_fields.uuids128 = adv_uuids128;
...
ble_app_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status != 0) {
            ESP_LOGW(TAG, "connect failed%s",
                     pairing_active ? "; re-advertise" : "; stay silent until BtnA");
            if (pairing_active) {
                ble_app_advertise();
            }
            return 0;
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        stop_ghost_kick_timer();
        advertising = false;
        bool was_app = audio_subscribed;
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        if (event->subscribe.attr_handle == audio_tx_char_handle) {
            ...
            ESP_LOGI(TAG, "Client %s audio notifications",
                     audio_subscribed ? "enabled" : "disabled");
            if (audio_subscribed) {
                pairing_active = false;
                stop_ghost_kick_timer();
                send_operation_mode_status();
            ...
            ESP_LOGI(TAG, "Client %s SMP OTA notifications",
                     smp_subscribed ? "enabled" : "disabled");
            if (smp_subscribed) {
                stop_ghost_kick_timer();
            }
            store_peer_mac();
...
void app_main(void)
{
    /* AXP192 must come up first or the rails (LCD, mic, 3.3 V) drop. */
    ESP_ERROR_CHECK(axp192_init());
    ESP_LOGI(TAG, "HarnessNode PlusSE (M5StickC Plus SE) - Starting");
    ...
    ESP_ERROR_CHECK(esp_timer_create(&bat_args, &battery_timer));
    ESP_ERROR_CHECK(esp_timer_start_periodic(battery_timer, BATTERY_POLL_INTERVAL_US));

    const esp_timer_create_args_t kick_args = {
        .callback = ghost_kick_cb,
        .name = "ghost_kick",
    };
    ESP_ERROR_CHECK(esp_timer_create(&kick_args, &ghost_kick_timer));
    ...
    ble_gatts_count_cfg(bas_svr_svcs);
    ble_gatts_count_cfg(smp_ota_svc_defs());
    ble_gatts_add_svcs(gatt_svr_svcs);
    ble_gatts_add_svcs(bas_svr_svcs);
